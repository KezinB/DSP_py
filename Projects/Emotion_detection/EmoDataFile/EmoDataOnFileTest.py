...
# Create a unique folder for storing data based on start time
start_time = datetime.now()
folder_name = f"EmoDataFile_{start_time.strftime('%Y%m%d_%H%M%S')}"
os.makedirs(folder_name, exist_ok=True)

# Create a text file to store emotion and time data
file_path = os.path.join(folder_name, "emotion_data.txt")

with open(file_path, 'w') as f:
    f.write("Time (s), Emotion\n")  # Header for the file
...
while True:
    ...
    try:
        ...
        with open(file_path, 'a') as f:
            f.write(f"{elapsed_time:.2f}, {emotion}\n")  # Log the time and emotion
