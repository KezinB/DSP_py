...
output_dir = r"C:\Users\kezin\OneDrive\Documents\Codes\python\Projects\phonocardiograms_PCG\final\final1\graphs"
...
# Function to compute and save log frequency power spectrum
def compute_and_save_log_spectrum(signal, sr, filename):
    # Dynamically adjust n_fft based on signal length
    n_fft = min(len(signal), 1024)  
    ...
    plt.title(f"Log Frequency Power Spectrum - {filename.split('.')[0]}")
    plt.xlabel('Time (s)')
...
# Step 5: Compute and save log spectrograms
compute_and_save_log_spectrum(maternal_pcg, rate, "original_log_spectrum.png")
compute_and_save_log_spectrum(noisy_pcg, rate, "noisy_log_spectrum.png")
compute_and_save_log_spectrum(recovered_pcg, rate, "recovered_log_spectrum.png")
