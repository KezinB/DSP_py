...
if test_mel_spec is not None:
    # Reshape for CNN (Add batch & channel dimensions)
    test_mel_spec = test_mel_spec[np.newaxis, ..., np.newaxis]  # Shape: (1, 128, 100, 1)

    # Make prediction
    probabilities = model.predict(test_mel_spec)[0]  # Output probability distribution
    prediction = np.argmax(probabilities)  # Get the predicted class (0 = Noise, 1 = Fetal Sound)
    ...
    print("Error: Test signal could not be processed.")
