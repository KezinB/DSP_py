...
class App:
    def __init__(self, root):
        ...
        #save
        self.upload_button = ttk.Button(self.main_frame, text="Save", command=self.showDetails,style="Rounded.TButton")
        ...
        self.resume_label.grid(row=5, column=0, columnspan=3, pady=10)
    ...
    def show_interview_ui(self):
        ...
        # Back Button
        back_button = ttk.Button(top_frame, text="Back", command=self.show_main_ui, style="Rounded.TButton")
        back_button.grid(row=0, column=0, sticky="w", padx=20, pady=10)
        
        # Video Feed Section
        video_frame = tk.Frame(top_frame, bg="#95D1CC")
        video_frame.grid(row=1, column=0, sticky="nsew")
        video_frame.grid_rowconfigure(0, weight=1)
        video_frame.grid_columnconfigure(0, weight=1)

        self.video_label = tk.Label(video_frame, bg="#000000")
        self.video_label.grid(row=0, column=0, padx=150, pady=10)
        
        # Video Controls Button (Start, End Video)
        start_video_button = ttk.Button(top_frame, text="Start Video", command=self.start_video, style="Rounded.TButton")
        start_video_button.grid(row=0, column=0, pady=5)

        end_video_button = ttk.Button(top_frame, text="End Video", command=self.end_video, style="Rounded.TButton")
        end_video_button.grid(row=0, column=1)
        # end_video_button.grid(row=0, column=1, pady=5)

        # Bottom Section (Video Feed & Controls)
        bottom_frame = tk.Frame(self.main_frame, bg=self.bg_color)
        bottom_frame.grid(row=1, column=0, sticky="nsew")
        bottom_frame.grid_rowconfigure(0, weight=6)
