...
class App:
    def __init__(self, root):
        ...
        self.start_interview_button = ttk.Button(self.main_frame, text="Start Interview", command=self.open_interview_window)
    ...
    def open_interview_window(self):
        # Open a new window for the interview
        self.interview_window = tk.Toplevel(self.root)
        self.interview_window.title("Interview Session")
        self.interview_window.geometry("800x600")
        self.interview_window.configure(bg="#f9f9f9")

        # Back Button
        back_button = ttk.Button(self.interview_window, text="Back", command=self.close_interview_window)
        back_button.pack(pady=10)

        # Video Label
        self.video_label = tk.Label(self.interview_window, bg="#f9f9f9")
        self.video_label.pack(pady=10)

        # Start Video Button
        start_video_button = ttk.Button(self.interview_window, text="Start Video", command=self.start_video)
        start_video_button.pack(pady=10)

        # End Video Button
        end_video_button = ttk.Button(self.interview_window, text="End Video", command=self.end_video)
        end_video_button.pack(pady=10)

    def close_interview_window(self):
        self.end_video()
        if self.interview_window:
            self.interview_window.destroy()
            self.interview_window = None
