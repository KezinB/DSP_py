...
phishing_weight = len(y_train) / (2 * np.sum(y_train))
legit_weight = len(y_train) / (2 * np.sum(1 - y_train))
weights = torch.tensor([legit_weight, phishing_weight], dtype=torch.float).to(device)
...
# %%
# Train the model
# n_epochs = 100 if device.type == "cuda" else 5
n_epochs = 30 if device.type == "cuda" else 5
print(f"\nTraining LSTM ({device.type.upper()} mode) for {n_epochs} epochs...")
...
torch.save(model.state_dict(), "lstm_phishing_model.pt")
...
for url in test_urls:
    ...
    print(f"{display_url:<45} | {'LSTM':<6} | {res_lstm:<10} | {conf_lstm:.4f}")
    print("-" * 70)