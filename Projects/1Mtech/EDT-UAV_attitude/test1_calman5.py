...
datafolder = os.path.join(os.path.dirname(__file__), "datafolder")
os.makedirs(datafolder, exist_ok=True)

# Generate filename with date and start time
start_dt = datetime.now()
csv_filename = start_dt.strftime("%Y-%m-%d_%H-%M-%S") + ".csv"
csv_path = os.path.join(datafolder, csv_filename)

# Open CSV file for logging (add buffering=1 for line buffering)
csv_file = open(csv_path, 'w', newline='', buffering=1)
csv_writer = csv.writer(csv_file)
csv_writer.writerow(['time', 'raw_roll', 'raw_pitch', 'kf_roll', 'kf_pitch'])
...
try:
    while True:
        ...
        # Inside main loop, after calculating values
        csv_writer.writerow([t, np.degrees(raw_roll), np.degrees(raw_pitch), smoothed_roll, smoothed_pitch])
