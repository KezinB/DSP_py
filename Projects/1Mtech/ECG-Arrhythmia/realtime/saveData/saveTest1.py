...
class ECGRecorder:
    ...
    def save_wfdb(self):
        ...
        with open(f"{self.base_name}.dat", 'wb') as f:
...
# Lead Configuration: Lead II"""
        with open(f"{self.base_name}.hea", 'w') as f:
            f.write(header)

        # Create empty annotation file
        with open(f"{self.base_name}.atr", 'w') as f:
...
<workbench_signals>
    <signal index="0">
        ...
        with open(f"{self.base_name}.xws", 'w') as f:
            f.write(xws_content)

        print(f"Saved {len(self.buffer)} samples to {self.base_name}.* files")
