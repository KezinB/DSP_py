...
class SerialPlotter(QtWidgets.QMainWindow):
    def __init__(self, serial_port, baud_rate=115200):
        ...
        # Plot widget
        self.plot_widget = pg.PlotWidget(title="Sensor Values")
...
def select_serial_port():
    ...
    item, ok = QtWidgets.QInputDialog.getItem(None, "Select Serial Port", "Available COM Ports:", ports, 0, False)
    if ok and item:
        return item.split()[0]  # Extract the port name (e.g., COM3)
    else:
        sys.exit()
 
if __name__ == "__main__":
    app = QtWidgets.QApplication(sys.argv)
    serial_port = select_serial_port()  # Show dialog to select serial port
