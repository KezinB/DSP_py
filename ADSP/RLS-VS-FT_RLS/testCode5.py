...
def rls(x, d, order, lambda_factor, delta):
    ...
    for n in range(order, N):
        ...
        if n % 100 == 0:  # Print every 100 iterations
            print(f"RLS Iteration {n}: y[{n}] = {y[n]}, e[{n}] = {e[n]}, w = {w}")
...
def ft_rls(x, d, order, lambda_factor, delta):
    ...
    for n in range(order, N):
        ...
        # Debugging output
        if n % 100 == 0:  # Print every 100 iterations
            print(f"FT-RLS Iteration {n}: y[{n}] = {y[n]}, e[{n}] = {e[n]}, w = {w}")
...
folder_name = "C:\\Users\\HP\\OneDrive\\Documents\\Codes\\python\\ADSP\\RLS-VS-FT_RLS"
